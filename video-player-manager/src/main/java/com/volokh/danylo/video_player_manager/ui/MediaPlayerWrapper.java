        ...
        MediaPlayer.OnVideoSizeChangedListener
{
    ...
    private final Runnable mOnVideoPreparedMessage = new Runnable() {
        @Override
        public void run() {
            Logger.v("TTT", "mOnVideoPreparedMessage run()");
            if (SHOW_LOGS) Logger.v("TTT", ">> run, onVideoPreparedMainThread");
    ...
    private final Runnable mOnVideoStartedMessage = new Runnable() {
        @Override
        public void run() {
            Logger.v("TTT", "mOnVideoStartedMessage run()");
            if (SHOW_LOGS) Logger.v(TAG, ">> run, mOnVideoStartedMessage");
    ...
    public void prepare() {
        ...
        synchronized (mState) {
            switch (mState.get()) {
                case STOPPED:
                case INITIALIZED:
                    Logger.v("TTTTT", "Start buffering INITIALIZED");
                    try {
                        if (mListener != null) {
                            Logger.v("TTT", "mListener != null");
                            mMainThreadHandler.post(mOnVideoPreparedMessage);
                        }
                        mMediaPlayer.prepare();
                        mState.set(State.PREPARED);
                        Logger.v("TTTTT", "Ready to play INITIALIZED");
                        //if (mListener != null) {
                        //    Logger.v("TTT", "mListener != null");
    ...
    private void printInfo(int what) {
        switch (what) {
            ...
            case MediaPlayer.MEDIA_INFO_VIDEO_RENDERING_START:
                ...
                if (mListener != null) {
                    Logger.v("TTT", "MEDIA_INFO_VIDEO_RENDERING_START != null");
                    mMainThreadHandler.post(mOnVideoStartedMessage);
                }
                break;
